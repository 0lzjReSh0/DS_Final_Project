...
def landlord_menu(stub):
    ...
    while True:
        ...
        if choice == '1':
            ...
            response = requests.get(f'http://localhost:5001/leave/receive', params={'recipient': username})
            ...
            region = input("Enter the region of the rental to delete: ")
            try:
                response = stub.DeleteRental(rent_pb2.RentalDeleteRequest(name=rental_name, username=username,region=region))
            ...
            owner = username
            if region not in ["A district", "B district"]:
                print("Currently the business only supports house rentals in district A and B. Please re-enter.")
            else:
    ...
    while True:
        ...
        if choice == '1':
            ...
            for i in response.entries:
                print(f"{i.name}, {i.price}, {i.description}")
                houses[i.name.lower()] = i
                
            house_name = input("Enter the name of the house to get more details, or type '#back' to return: ")
            if house_name == '#back':
                continue
            elif house_name not in houses:
                print("No such house found. Please enter a valid house name.")
            else:
                ...
                if detail_response.name:
                    ...
                    response = requests.post('http://localhost:5001/leave/send', json={
                        'sender': username,
                        'recipient': recipient,
                        'content': message
                    })
