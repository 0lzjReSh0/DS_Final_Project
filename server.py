...
def get_messages():
    ...
    with psycopg2.connect(host="localhost", database="DS", user="postgres", password="020603") as conn:
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT sender, content, timestamp FROM messages WHERE recipient = %s AND read = FALSE",
                (recipient,)
            )
...
class RentalServiceServicer(rent_pb2_grpc.RentalServiceServicer):
    ...
    def DeleteRental(self, request, context):
        try:
            self.cursor.execute("DELETE FROM rent WHERE name = %s AND owner = %s", (request.name, request.username))
            self.conn.commit()
    ...
    def AddRental(self, request, context):
        try:
            self.cursor.execute(
                "INSERT INTO rent (name, price, location, owner, description, region) VALUES (%s, %s, %s, %s, %s,%s)",
    ...
    def GetAllMessages(self, request, context):
        ...
        try:
            self.cursor.execute(
                "SELECT sender, content, timestamp FROM messages WHERE recipient = %s",
                (request.username,)
            )
    ...
    def GetMyRentals(self, request, context):
        try:
            ...
            self.cursor.execute("SELECT name, price, location, description FROM rent WHERE owner = %s", (request.username,))
            rows = self.cursor.fetchall()
    ...
    def GetRentalInfo(self, request, context):
        try:
            query = "SELECT name, price, location, region FROM rent WHERE region = %s"
            if request.region == "all":
                query = "SELECT name, price, location FROM rent"
            self.cursor.execute(query, (request.region,))
            rows = self.cursor.fetchall()
            entries = [rent_pb2.RentalEntry(name=row[0], price=row[1], description=row[2]) for row in rows]
